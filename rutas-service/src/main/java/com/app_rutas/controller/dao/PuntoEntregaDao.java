...
public class PuntoEntregaDao extends AdapterDao<PuntoEntrega> {
    ...
    public Boolean update() throws Exception {
        ...
        Integer index = getPuntoEntregaIndex("id", this.puntoEntrega.getId());
        if (index == -1) {
    ...
    private LinkedList<PuntoEntrega> linearBinarySearch(String attribute, Object value) throws Exception {
        ...
        LinkedList<PuntoEntrega> puntoEntregas = new LinkedList<>();
        if (!lista.isEmpty()) {
    ...
    public PuntoEntrega buscarPor(String attribute, Object value) throws Exception {
        ...
        if (!lista.isEmpty()) {
            PuntoEntrega[] puntoEntregas = lista.toArray();
            for (int i = 0; i < puntoEntregas.length; i++) {
                if (obtenerAttributeValue(puntoEntregas[i], attribute).toString().toLowerCase()
    ...
    private Integer getPuntoEntregaIndex(String attribute, Object value) throws Exception {
        if (this.listAll == null) {
            this.listAll = listAll();
        }
        Integer index = -1;
        if (!this.listAll.isEmpty()) {
            PuntoEntrega[] puntoEntregas = this.listAll.toArray();
            for (int i = 0; i < puntoEntregas.length; i++) {
                if (obtenerAttributeValue(puntoEntregas[i], attribute).toString().toLowerCase()
                        .equals(value.toString().toLowerCase())) {
                    index = i;
    ...
    public String getPuntoEntregaJasonByIndex(Integer index) throws Exception {
        Gson g = new Gson();
        return g.toJson(get(index));
    }


    public String getPuntoEntregaJson(Integer Index) throws Exception {
        Gson g = new Gson();
        return g.toJson(get(Index));
    }
